 ...
 * 작업들을 자동으로 수행할 수 있도록 미리 설정하는 것이다.
 */
 ...
 * 내부에서 프로그램 종료부인 ;을 추가하게 되는데 이 때 정상적으로 동작하지 않는 문제가 발생되어
 * 구분자를 ;에서 다른 것으로 변경 처리하고 내부 기능을 정의할 수 있도록 하기 위해서 사용한다.
 */

/* 행 트리거
 * 테이블 안에 영향을 받는 각각의 형에 대하여 실행된다.
 * old - 변경 전의 데이터로 delete 혹은 upgrade로 변경되기 전의 데이터를 의미한다.
 * new - 변경 후 데이터로 insert 혹은 upgrade로 변경된 데이터를 의미한다.
 * 
 * old : insert는 변경 전의 데이터가 아니기 때문에 old에서 감지할 수 없다.
...
AFTER INSERT on tbl_order -- {AFTER | befor, INSERT | UPDATE | DELETE)
FOR EACH ROW
BEGIN 
